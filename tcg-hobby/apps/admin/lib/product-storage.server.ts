...
function detectImageType(buffer: Buffer): 'jpeg' | 'png' | 'webp' | 'avif' | null {
  if (buffer.length >= 12 && buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff) return 'jpeg';
  if (buffer.length >= 8 && buffer.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))) return 'png';
  if (buffer.length >= 12 && buffer.toString('ascii', 0, 4) === 'RIFF' && buffer.toString('ascii', 8, 12) === 'WEBP') return 'webp';
  if (buffer.length >= 12 && buffer.toString('ascii', 4, 8) === 'ftyp' && ['avif', 'avis'].includes(buffer.toString('ascii', 8, 12))) return 'avif';
  return null;
}

async function processManagedImage(
  prefixRoot: 'products' | 'heroes',
  ownerId: string,
  file: File,
  mainBounds: { width: number; height: number },
): Promise<ProcessedProductImage> {
  if (!file.size || file.size > MAX_PRODUCT_IMAGE_BYTES) throw new Error('Choose a non-empty JPEG, PNG, WebP or AVIF image no larger than 10 MB.');
  const source = Buffer.from(await file.arrayBuffer());
  if (!detectImageType(source)) throw new Error('The file contents are not a supported JPEG, PNG, WebP or AVIF image.');
  ...
  const prefix = `${prefixRoot}/${ownerId}/${randomUUID()}`;
  const main = await sharp(source).rotate().resize({ ...mainBounds, fit: 'inside', withoutEnlargement: true }).webp({ quality: 88 }).toBuffer();
...
export function processProductImage(productId: string, file: File) {
  return processManagedImage('products', productId, file, { width: 2400, height: 2400 });
}
export function processHeroImage(placementId: string, file: File) {
  return processManagedImage('heroes', placementId, file, { width: 2560, height: 1440 });
}
