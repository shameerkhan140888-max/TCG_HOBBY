...
export class PublicCommerceService {
  ...
  async catalogue(query: Record<string, string | undefined>): Promise<PublicCatalogueResponse> {
    ...
    const filters: CatalogueFilters = {
      ...
      pieceCount: query.pieceCount?.trim() ?? '',
      structure: query.structure?.trim() ?? '',
      buildType: query.buildType?.trim() ?? '',
      vehicleManufacturer: query.vehicleManufacturer?.trim() ?? '',
      availability: query.availability?.trim() ?? '',
      offers: query.offers?.trim() ?? '',
