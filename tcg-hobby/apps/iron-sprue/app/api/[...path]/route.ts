...
import { copyProxyRequestHeaders, getNodeApiOrigin, isAllowedProxyRoute, signInternalRequest } from '../../../lib/node-proxy';
...
async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  const secret = process.env.IRON_SPRUE_INTERNAL_API_SECRET;
  if (!secret) {
  ...
  headers.set('x-iron-sprue-internal-signature', await signInternalRequest({ method: request.method, pathname, timestamp, nonce, body, secret }));
  headers.set('x-iron-sprue-store', 'IRON_SPRUE');
