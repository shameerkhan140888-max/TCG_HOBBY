...
const paidOrder: PublicOrderDetail = {
...
describe('Iron Sprue checkout result state', () => {
  ...
  it('keeps a refreshed confirmation page confirmed when the API briefly returns no initial order', () => {
    ...
    window.sessionStorage.setItem('iron-sprue:checkout-success:session:cs_test_1', JSON.stringify(paidOrder));
    ...
    expect(markup).toContain('Order <span class="order-reference-heading">IS-20260824-ABC123</span> confirmed');
    expect(markup).not.toContain('Processing your payment');
  });
});
