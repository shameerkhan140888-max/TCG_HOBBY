...
- Email: `IRON_SPRUE_RESEND_API_KEY`, `IRON_SPRUE_EMAIL_FROM`, `IRON_SPRUE_EMAIL_REPLY_TO`, `IRON_SPRUE_SUPPORT_EMAIL`, `IRON_SPRUE_EMAIL_ASSET_BASE_URL`, `IRON_SPRUE_EMAIL_LOGO_URL`.
...
- Receipt/order confirmation, cancellation/refund and dispatch/tracking templates share the Iron Sprue header/branding template; if `IRON_SPRUE_EMAIL_LOGO_URL` is not supplied, emails fall back to the approved horizontal Iron Sprue email-safe PNG logo at `${IRON_SPRUE_EMAIL_ASSET_BASE_URL}/brand/iron-sprue-horizontal-email.png`.
- Product images in customer emails use `IRON_SPRUE_EMAIL_ASSET_BASE_URL` for persisted relative media paths. This must be a public HTTPS Iron Sprue asset host; inboxes cannot render `localhost` image URLs.
