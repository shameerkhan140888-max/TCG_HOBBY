...
function planningDb(products: Array<{ id: string; sku: string; barcode: string | null; slug: string }> = []) {
  return {
    ...
    game: {
      findMany: vi.fn().mockResolvedValue([{ id: 'game-1', name: 'Pok̩mon TCG', slug: 'pokemon-tcg', active: true, sortOrder: 10, _count: { products: 0 } }]),
    },
    brand: {
...
describe('product CSV import', () => {
  ...
  it('reports missing canonical lookup data clearly', async () => {
    const db = {
      ...
      game: { findMany: vi.fn().mockResolvedValue([{ id: 'game-1', name: 'Pok̩mon TCG', slug: 'pokemon-tcg', active: true, sortOrder: 10, _count: { products: 0 } }]) },
  ...
  it('rejects set values that belong to another game', async () => {
    ...
    expect(plan.summary.errorRows).toBe(1);
    expect(plan.rows[0]?.errors).toContain('Set "Magic Set" does not belong to Game "Pok̩mon TCG".');
  });
